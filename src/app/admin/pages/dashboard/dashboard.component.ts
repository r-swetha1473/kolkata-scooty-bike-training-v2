...
@Component({
  ...
  template: `
    <div class="dashboard">
      <div class="admin-page-header dashboard-header">
        ...
        <button type="button" class="admin-btn admin-btn-secondary" (click)="loadStats()" [disabled]="loading">Refresh</button>
      </div>
      ...
      <div class="kpi-grid motion-stagger motion-visible" *ngIf="!loading">
  ...
  styles: [
    `
      ...
      .dashboard-header,
      .admin-page-header.dashboard-header {
        display: flex;
        flex-wrap: wrap;
        justify-content: space-between;
        ...
        padding: 0 0 1.15rem;
        border-bottom: 1px solid var(--admin-border-light, #E8EEF4);
        background: none;
        color: inherit;
        box-shadow: none;
        border-radius: 0;
      }
      .dashboard-header .admin-page-title {
        color: var(--admin-text, #08111F);
      ...
      .dashboard-subtitle {
        margin: 6px 0 0;
        color: var(--admin-text-secondary, #5A6876);
        font-size: 0.9rem;
      }
