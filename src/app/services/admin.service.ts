...
export class AdminService {
  ...
  async getSchedulingHealth(): Promise<any> {
    return firstValueFrom(this.http.get<any>('/admin/scheduling-health'));
  }
  ...
  async createOfflineBookingFormData(formData: FormData): Promise<any> {
    return firstValueFrom(this.http.postFormData<any>('/admin/offline-bookings', formData));
  }
  ...
  async getAllTrainers(branchId?: string): Promise<any[]> {
    const qs = branchId ? `?branch_id=${encodeURIComponent(branchId)}` : '';
    const result = await firstValueFrom(this.http.get<any[]>(`/admin/trainers${qs}`));
    return result || [];
  }
  ...
  addTrainerLeave(trainerId: string, leaveDate: string, reason?: string): Observable<any> {
    return this.http.post(`/admin/trainers/${trainerId}/leave`, {
      leave_date: leaveDate,
      ...(reason ? { reason } : {})
    });
  }

  restoreTrainerLeave(trainerId: string, leaveDate: string): Observable<any> {
    return this.http.delete(`/admin/trainers/${trainerId}/leave/${leaveDate}`);
  }

  getTrainerLeaveDates(trainerId: string): Observable<{ leave_dates: Array<{ leave_date: string; reason?: string }> }> {
    return this.http.get(`/admin/trainers/${trainerId}/leave`);
  }
  ...
  async getUserActivity(userId: string, params?: { search?: string; limit?: number; offset?: number }) {
    const qs = new URLSearchParams();
    if (params?.search) qs.set('search', params.search);
    if (params?.limit != null) qs.set('limit', String(params.limit));
    if (params?.offset != null) qs.set('offset', String(params.offset));
    const suffix = qs.toString() ? `?${qs}` : '';
    return firstValueFrom(this.http.get<any>(`/admin/users/${userId}/activity${suffix}`));
  }
