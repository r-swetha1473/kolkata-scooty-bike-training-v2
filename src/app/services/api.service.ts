...
export class ApiService {
  ...
  createBooking(
    ...
    if (options?.branch_id) payload.branch_id = options.branch_id;
    if (options?.course_id) payload.course_id = options.course_id;
    const coupon = options?.coupon_code?.trim();
  ...
  getMyProgress(): Observable<{ enrollments: any[]; source?: string }> {
    return this.http.get<{ enrollments: any[]; source?: string }>(`${this.apiUrl}/profiles/me/progress`, {
      headers: this.getAuthHeaders(),
      withCredentials: true
    });
  }
