  ...
  try {
    ...
    if (isCustomer) {
      ...
      if (mode === 'create') {
        ...
        const activeTotal = parseInt(activeTotalResult.rows[0]?.count || 0, 10);
        if (activeTotal >= TOTAL_BOOKING_LIMIT) {
          return {
            eligible: false,
            reason: 'TOTAL_LIMIT_REACHED',
            message: `You already have ${activeTotal} active booking(s). Maximum allowed: ${TOTAL_BOOKING_LIMIT}.`,
            details: { activeCount: activeTotal, limit: TOTAL_BOOKING_LIMIT }
          };
        }
      }

      // Check 3: Configurable gap between customer bookings
      if (BOOKING_GAP_HOURS > 0) {
      ...
      if (gapConflict.rows.length > 0) {
        ...
        return {
          ...
          message: `You already have a booking within the last ${BOOKING_GAP_HOURS} hours. Your next booking can be made after ${formatKolkataDateTime(nextAllowed)}.`,
      ...
      if (advanceCheck.rows[0]?.too_soon) {
        return {
          ...
          message: bookingRulesSvc.bookingAdvanceMessage(MIN_BOOKING_ADVANCE_HOURS),
          details: {
            minAdvanceHours: MIN_BOOKING_ADVANCE_HOURS,
    ...
    // Booking window: slot must be in the future and within configured advance window
    if (hoursUntilSlot <= 0) {
      return {
        eligible: false,
        reason: 'SLOT_PAST',
    ...
    if (hoursUntilSlot > bookingWindowHours) {
      return {
        eligible: false,
        reason: 'BOOKING_NOT_OPEN_YET',
        message: bookingRulesSvc.bookingWindowMessage(bookingWindowHours),
        details: { hoursUntilSlot, bookingWindowHours }
      };
    }
    ...
    if (slotId) {
      ...
      // Database-driven window already loaded via getBookingRules() above
      const visibilityHours = bookingWindowHours;
      const isCurrentlyVisible =
        hoursUntil > 0 && hoursUntil <= visibilityHours;

      if (!slot.is_visible && !isCurrentlyVisible) {
        return {
          eligible: false,
          reason: 'SLOT_NOT_VISIBLE',
          message: bookingRulesSvc.bookingWindowMessage(visibilityHours),
          details: { slotStartTime: slot.start_time, hoursUntil, visibilityHours }
        };
      }
...
async function validateCancellationEligibility(phone, bookingId) {
  try {
    ...
    // Check cancellation window (configurable hours before slot)
    const slotStartTime = new Date(booking.start_time);
    const currentTime = new Date();
    const hoursUntilSlot = (slotStartTime - currentTime) / (1000 * 60 * 60);
    const { cancellationWindowHours: CANCELLATION_WINDOW_HOURS } = await getBookingRules();
    
    if (hoursUntilSlot <= CANCELLATION_WINDOW_HOURS) {
      return {
        eligible: false,
        reason: 'CANCELLATION_WINDOW_PASSED',
        message: bookingRulesSvc.cancellationWindowMessage(CANCELLATION_WINDOW_HOURS),
