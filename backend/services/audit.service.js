...
async function logAdminAction({
  ...
  try {
    ...
    // Older DBs without ip/ua columns — fall back
    if (String(error.message || '').includes('ip_address') || String(error.message || '').includes('user_agent')) {
      try {
        await db.query(
        ...
        console.error('[Audit] Failed to log admin action (fallback):', fallbackErr);
        return;
      }
    }
...
async function logAuthEvent(userId, actionType, details = {}, ipAddress = null, userAgent = null) {
  await logAdminAction({
    adminId: userId || null,
    actionType,
    entityType: 'auth',
...
async function logPaymentApproval(adminId, payment, beforeStatus = 'pending_verification') {
  await logAdminAction({
    adminId,
    actionType: 'PAYMENT_APPROVED',
    entityType: 'payment',
    entityId: payment.id,
    beforeValue: { status: beforeStatus, booking_id: payment.booking_id },
    afterValue: { status: 'verified', booking_id: payment.booking_id, booking_status: 'confirmed' },
    details: { booking_id: payment.booking_id }
  });
}

async function logPaymentRejection(adminId, payment, reason, beforeStatus = 'pending_verification') {
  await logAdminAction({
    adminId,
    ...
    entityId: payment.id,
    beforeValue: { status: beforeStatus, booking_id: payment.booking_id },
    afterValue: { status: 'rejected', booking_id: payment.booking_id, rejection_reason: reason || null },
    details: { booking_id: payment.booking_id }
  });
}
