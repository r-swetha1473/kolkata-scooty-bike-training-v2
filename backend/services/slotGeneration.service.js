...
const bookingWindowSvc = require('./bookingWindow.service');
...
async function fetchActiveTrainerIds(client, branchId = null) {
  if (branchId) {
    const r = await client.query(
      `SELECT id FROM trainers WHERE is_active = true AND branch_id = $1
...
function buildSlotTemplatesForDate(
  dateString,
  trainerIds,
  ...
  if (branchSchedule?.working_days && Array.isArray(branchSchedule.working_days)) {
    if (!branchSchedule.working_days.map(Number).includes(dayOfWeek)) {
  ...
  const intervalMinutes =
    branchSchedule?.slot_duration_minutes ||
    config.slot.generation.weekday.intervalMinutes;

  const pushSlot = (slotStart, slotEnd) => {
    const now = new Date();
    const visibilityHours = bookingWindowSvc.getBookingWindowHoursSync();
    const visibilityThreshold = new Date(now.getTime() + visibilityHours * 60 * 60 * 1000);
  ...
  // Prefer branch opening/closing when provided; else legacy weekday/sunday templates
  if (branchSchedule?.opening_time && branchSchedule?.closing_time && dayOfWeek !== 0) {
    const [oh, om] = String(branchSchedule.opening_time).split(':').map(Number);
    const [ch, cm] = String(branchSchedule.closing_time).split(':').map(Number);
    let currentStartMinutes = oh * 60 + (om || 0);
    ...
    while (currentStartMinutes + intervalMinutes <= endMinutes) {
      ...
      const startUTC = convertISTToUTC(Math.floor(currentStartMinutes / 60), currentStartMinutes % 60);
      const endUTC = convertISTToUTC(Math.floor(slotEndMinutes / 60), slotEndMinutes % 60);
    ...
    const step = intervalMinutes || weekdayConfig.intervalMinutes;
    let currentStartMinutes = startMinutes;

    while (currentStartMinutes + step <= endMinutes) {
      const slotStartMinutes = currentStartMinutes;
      const slotEndMinutes = currentStartMinutes + step;
 ...
 * @param {string|null} [options.actorProfileId]
 * @param {string} [options.branchId] required for branch-scoped generation
 */
async function generateSlotsForDate(dateString, options = {}) {
  ...
  try {
    ...
    if (!branchId) {
      ...
      releaseOnce();
      const error = new Error('No active branch found. Create a branch first.');
      error.status = 400;
      error.errorCode = 'NO_BRANCH';
      throw error;
    }

    const branchRow = await client.query(`SELECT * FROM branches WHERE id = $1`, [branchId]);
    const branchSchedule = branchRow.rows[0] || null;
    ...
    if (mode === 'admin') {
      const existingSlotsCheck = await client.query(
        ...
        FROM slots
        WHERE slot_date = $1::date AND is_auto_generated = true AND branch_id = $2
        `,
        [dateNorm, branchId]
      );
      ...
      if (existingCount > 0 && !force) {
        ...
        while (attempts < maxAttempts) {
          const checkResult = await db.query(
            `SELECT COUNT(*) as count FROM slots WHERE slot_date = $1::date AND is_auto_generated = true AND branch_id = $2`,
            [nextDate, branchId]
          );
    ...
    const vehicles = await vehicleService.getActiveVehicles(branchId);
    if (vehicles.length === 0) {
      await client.query('ROLLBACK');
      releaseOnce();
      const error = new Error('No active vehicles found for this branch. Please create vehicles first.');
      error.status = 400;
      error.errorCode = 'NO_VEHICLES';
      throw error;
    }

    const trainerIds = await fetchActiveTrainerIds(client, branchId);
    const slotCapacity = await slotCapacityService.resolveSlotCapacity(client, branchId);
    const { slots, dayName } = buildSlotTemplatesForDate(
      dateNorm,
      trainerIds,
      slotCapacity,
      branchSchedule
    ...
    for (const slot of slots) {
      try {
        const existing = await client.query(
          ...
          FROM slots
          WHERE slot_date = $1 AND start_time = $2 AND branch_id = $3
          LIMIT 1
          `,
          [slot.slot_date, slot.start_time, branchId]
        );
        ...
        if (existing.rows.length > 0) {
          ...
          const insertResult = await client.query(
            `
            INSERT INTO slots (trainer_id, start_time, end_time, capacity, booked_count, status, slot_date, is_auto_generated, is_visible, branch_id)
    ...
    const slotsToUpdate = await client.query(
      `SELECT id FROM slots WHERE slot_date = $1::date AND branch_id = $2`,
      [dateNorm, branchId]
    );
    ...
    await client.query(
      ...
      WHERE slot_date = $2::date
        ...
        AND booked_count <= $1
      `,
      [slotCapacity, dateNorm, branchId]
    );
...
/**
 * Nightly job: for each active branch, ensure today through next 6 days have slots.
 */
async function runNightlyAutoGeneration() {
  ...
  if (branchIds.length === 0) {
    return { success: false, results, errors: [{ message: 'No active branches' }] };
  }
  ...
  for (const branchId of branchIds) {
    for (let i = 0; i < 7; i++) {
      ...
      try {
        const r = await generateSlotsForDate(dateStr, { mode: 'auto', force: false, branchId });
        results.push(r);
      } catch (e) {
        errors.push({ date: dateStr, branchId, message: e.message, code: e.errorCode });
      }
    }
  }
