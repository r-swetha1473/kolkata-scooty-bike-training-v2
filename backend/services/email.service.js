...
class EmailService {
  ...
  async sendEmail(to, subject, html, text) {
    ...
    const brandedHtml = `
      <div style="font-family:Inter,Segoe UI,Arial,sans-serif;max-width:560px;margin:0 auto;color:#0F172A">
        <div style="padding:16px 0;border-bottom:1px solid #E5E7EB;margin-bottom:16px">
          <img src="${process.env.PUBLIC_SITE_URL || process.env.SITE_URL || 'https://kolkata-scooty-bike-training.vercel.app'}/assets/brand/logo.png" alt="Kolkata Scooty Bike Training" height="40" style="height:40px;width:auto;display:block;margin-bottom:8px" />
          <strong style="color:#2563EB;font-size:18px">Kolkata Scooty Bike Training</strong>
          <div style="color:#64748B;font-size:12px;margin-top:4px">Safe · Professional · Women-friendly riding school</div>
        </div>
        ${html}
        <p style="margin-top:28px;font-size:12px;color:#64748B">© Kolkata Scooty Bike Training · Kolkata, West Bengal</p>
      </div>`;
  ...
  async sendBookingConfirmation(booking, user, slot, trainer, vehicle) {
    const subject = 'Booking Confirmed - Kolkata Scooty Bike Training';
    const html = `
      ...
      <p>Thank you for choosing Kolkata Scooty Bike Training!</p>
    `;
  ...
  async sendBookingCancellation(booking, user, slot, trainer, vehicle) {
    const subject = 'Booking Cancelled - Kolkata Scooty Bike Training';
    const html = `
  ...
  async sendAdminInactivityBlockAlert(users, days) {
    ...
    const subject = `[Kolkata Scooty] ${users.length} customer(s) blocked for ${days}-day inactivity`;
    const html = `
  ...
  async sendTrainerChangeNotification(booking, user, slot, oldTrainer, newTrainer, vehicle) {
    const subject = 'Trainer Changed - Kolkata Scooty Bike Training';
    const html = `
