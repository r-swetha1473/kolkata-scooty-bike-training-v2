...
const validateOfflineBookingCreation = [
  ...
  body('payment_mode')
    .optional({ values: 'falsy' })
    .isString()
    .isIn(['pending', 'complete_now', 'complete', 'paid_now'])
    .withMessage('payment_mode must be pending or complete_now'),
  ...
  body('amount_paid')
    .optional({ values: 'falsy' })
    .isFloat({ min: 0 })
    .withMessage('amount_paid must be a non-negative number'),
  ...
  body('payment_status')
    ...
    .withMessage('payment_status must be pending, paid, partial, or failed'),
