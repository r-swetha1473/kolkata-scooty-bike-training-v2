...
const {
  ...
  validateTrainerLeave,
  validateTrainerLeaveRestore,
...
const { getSchedulingEngineHealth } = require('../services/schedulingHealth.service');
const cloudinaryService = require('../services/cloudinary.service');
const { createImageUploader } = require('../middleware/cloudinaryUpload');
const { jsonError } = require('../utils/httpError');

const trainerImageUpload = createImageUploader({
  folder: 'trainers',
  maxBytesEnv: 'TRAINER_IMAGE_MAX_BYTES'
});
...
const offlinePaymentUpload = multer({
...
const trainerLeaveService = require('../services/trainerLeave.service');
const { formatTrainerRow, TRAINER_LIST_SELECT } = require('../utils/trainerFormat');
...
function bookingListFiltersFromQuery(query) {
  const branchId = query.branchId || query.branch_id;
  const trainerId = query.trainerId || query.trainer_id;
  const vehicleId = query.vehicleId || query.vehicle_id;
  const paymentStatus = query.paymentStatus || query.payment_status;
  return {
    status: query.status ? String(query.status).trim() : '',
    source: query.source ? String(query.source).trim().toUpperCase() : '',
    attendance: query.attendance ? String(query.attendance).trim() : '',
    startDate: query.startDate ? String(query.startDate).trim() : '',
    endDate: query.endDate ? String(query.endDate).trim() : '',
    searchRaw: query.search != null ? String(query.search).trim() : '',
    branchId: branchId ? String(branchId).trim() : '',
    trainerId: trainerId ? String(trainerId).trim() : '',
...
router.get('/bookings', requirePermission('bookings', 'view'), async (req, res, next) => {
  try {
    const { countSql, listSql, countParams, listParams, limit, offset } = buildBookingListQuery({
      ...bookingListFiltersFromQuery(req.query),
...
router.get('/bookings/export', requirePermission('bookings', 'view'), async (req, res, next) => {
  try {
    ...
    const exportRows = result.rows.map((row) => {
      ...
      return {
        'Reference Number': row.booking_reference || row.offline_reference_number || row.id,
...
router.post(
  ...
  requirePermission('bookings', 'create'),
  (req, res, next) => {
    offlinePaymentUpload.single('payment_proof')(req, res, (err) => {
      if (err) {
  ...
  async (req, res, next) => {
    try {
      const booking = await offlineBookingService.createOfflineBooking(req.user.id, {
        ...req.body,
        payment_proof: req.file || null
      });
...
router.get('/users/:id/activity', requirePermission('users', 'view'), async (req, res, next) => {
  try {
    const search = req.query.search != null ? String(req.query.search).trim() : '';
    const limit = Math.min(parseInt(req.query.limit, 10) || 50, 200);
    const offset = Math.max(parseInt(req.query.offset, 10) || 0, 0);
    const payload = await userActivityService.getUserActivity(req.params.id, { search, limit, offset });
    res.json(payload);
  } catch (error) {
    next(error);
  }
...
/** Scheduling engine + platform health (admin monitoring) */
router.get('/scheduling-health', requirePermission('dashboard', 'view'), async (req, res, next) => {
  try {
    const health = await getSchedulingEngineHealth();
    res.json(health);
  } catch (error) {
    next(error);
  }
...
router.get('/trainers', requirePermission('trainers', 'view'), async (req, res, next) => {
  try {
    ...
    res.json(result.rows.map(formatTrainerRow));
  } catch (error) {
    next(error);
  }
});

function mapTrainerConstraintError(error, next) {
  if (error.code === '23505') {
    const detail = String(error.detail || '');
    const isPhone = detail.includes('phone');
    const err = new Error(isPhone ? 'Phone number already exists' : 'Email already exists');
    err.status = 409;
    err.errorCode = isPhone ? 'DUPLICATE_PHONE' : 'DUPLICATE_EMAIL';
    return next(err);
  }
  if (error.code === '23503' && String(error.constraint || '').includes('branch')) {
    const err = new Error('Invalid branch selected');
    err.status = 400;
...
async function assertActiveBranch(branchId) {
  ...
  if (!branchCheck.rows.length) {
    const error = new Error('Invalid or inactive branch selected');
    error.status = 400;
...
router.post('/trainers', requirePermission('trainers', 'create'), validateTrainerCreation, async (req, res, next) => {
  try {
    const { email, full_name, phone, bio, experience_years, specialization, rating, branch_id } = req.body;

    await assertActiveBranch(branch_id);
    ...
    try {
      ...
      const finalPhone = phone && String(phone).trim()
        ? String(phone).trim()
        : `TRAINER_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      const trainerResult = await client.query(
        `INSERT INTO trainers (user_id, bio, experience_years, specialization, is_active, rating, branch_id)
         ...
         RETURNING *`,
        [userId, bio, experience_years || 0, specialization || [], rating != null ? rating : 0, branch_id]
      );
    ...
    mapTrainerConstraintError(error, next);
  }
});

/** Upload trainer avatar → kolkata-bike-training/trainers, stores on profiles.avatar_url */
router.post(
  '/trainers/:id/upload-image',
  requirePermission('trainers', 'edit'),
  trainerImageUpload.middleware,
  async (req, res, next) => {
    try {
      ...
      const trainer = await db.query(
        `SELECT t.id, t.user_id, p.avatar_url
         FROM trainers t JOIN profiles p ON p.id = t.user_id
         WHERE t.id = $1`,
        [req.params.id]
      );
      if (!trainer.rows[0]) {
        cloudinaryService.unlinkQuiet(req.file.path);
        return jsonError(res, 404, 'Trainer not found', 'TRAINER_NOT_FOUND');
      }

      const uploaded = await cloudinaryService.uploadImage(req.file, {
        folder: 'trainers',
        maxBytes: trainerImageUpload.maxBytes
      });

      await db.query(`UPDATE profiles SET avatar_url = $1, updated_at = NOW() WHERE id = $2`, [
        uploaded.secure_url,
        trainer.rows[0].user_id
      ]);
      await cloudinaryService.replaceImage(trainer.rows[0].avatar_url, uploaded.secure_url);
...
router.put('/trainers/:id', requirePermission('trainers', 'edit'), validateTrainerUpdate, async (req, res, next) => {
  try {
    const { id } = req.params;
    const { is_active, bio, experience_years, specialization, full_name, phone, rating, branch_id } = req.body;
    ...
    if (updates.length === 0 && full_name === undefined && phone === undefined) {
    ...
    const beforeResult = await db.query(
      `SELECT t.id, t.bio, t.experience_years, t.specialization, t.rating, t.is_active, t.branch_id, p.full_name, p.phone
...
// Mark trainer leave for a date
router.post('/trainers/:id/leave', requirePermission('trainers', 'edit'), validateTrainerLeave, async (req, res, next) => {
  try {
    const leaveDate = String(req.body.leave_date).slice(0, 10);
    const leave = await trainerLeaveService.addLeaveDate(req.params.id, leaveDate, req.body.reason || null);
    const result = await db.query(`${TRAINER_LIST_SELECT} WHERE t.id = $1`, [req.params.id]);
    const trainer = formatTrainerRow(result.rows[0]);
    if (trainer.branch_id) {
      invalidateCacheForBranch(trainer.branch_id);
    }
    res.status(201).json({
...
// Restore trainer from leave (remove leave date)
router.delete('/trainers/:id/leave/:date', requirePermission('trainers', 'edit'), validateTrainerLeaveRestore, async (req, res, next) => {
  try {
    const leave = await trainerLeaveService.removeLeaveDate(req.params.id, req.params.date);
    const result = await db.query(`${TRAINER_LIST_SELECT} WHERE t.id = $1`, [req.params.id]);
    const trainer = formatTrainerRow(result.rows[0]);
    if (trainer.branch_id) {
      invalidateCacheForBranch(trainer.branch_id);
...
// List trainer leave dates
router.get('/trainers/:id/leave', requirePermission('trainers', 'view'), async (req, res, next) => {
  try {
    const trainerCheck = await db.query('SELECT id FROM trainers WHERE id = $1', [req.params.id]);
    ...
    const leaveDates = await trainerLeaveService.listLeaveDates(req.params.id);
    res.json({ leave_dates: leaveDates });
  } catch (error) {
    next(error);
  }
});
...
router.delete('/trainers/:id', requirePermission('trainers', 'delete'), validateTrainerDelete, async (req, res, next) => {
  try {
    ...
    const avatarRow = await db.query(
      `SELECT p.avatar_url FROM trainers t JOIN profiles p ON p.id = t.user_id WHERE t.id = $1`,
      [req.params.id]
    );
    ...
    await cloudinaryService.destroyImage(avatarRow.rows[0]?.avatar_url);
    res.json(result);
  } catch (error) {
    if (error.code === '23503') {
