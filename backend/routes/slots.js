...
const bookingWindowSvc = require('../services/bookingWindow.service');
...
router.get('/', async (req, res, next) => {
  try {
    const { trainer_id, start_date, end_date, date, status, available_only, branch_id } = req.query;
...
router.get('/date/:date', async (req, res, next) => {
  try {
    ...
    try {
      ...
      if (!tableExists.rows[0]?.exists) {
        const migrationError = new Error(
          ...
          'Please ensure database/schema.sql has been applied (see DATABASE.md).'
        );
    ...
    const dateQuery = `
      ...
      LEFT JOIN slot_vehicle_capacity svc ON svc.slot_id = s.id
      LEFT JOIN vehicles v ON svc.vehicle_id = v.id AND v.is_active = true
        AND (s.branch_id IS NULL OR v.branch_id = s.branch_id)
      LEFT JOIN LATERAL (
        SELECT COUNT(*) as booked_count
        FROM bookings b
        WHERE b.slot_id = s.id AND b.vehicle_id = v.id AND b.status NOT IN ('cancelled')
      ...
      ${req.query.branch_id ? `AND s.branch_id = $2` : ''}
    ...
    const dateParams = req.query.branch_id ? [date, req.query.branch_id] : [date];
    const result = await ensureAutoSlotsForDateIfNeeded(dateQuery, dateParams, date, 'date');
    ...
    const enriched = enrichSlotsWithLiveCapacity(result.rows).map((row) => {
      const capacity = Number(row.live_capacity ?? row.capacity ?? 0);
...
router.post('/', ...adminAccess('slots', 'create'), validateSlotCreation, async (req, res, next) => {
  try {
    ...
    let branchId = bodyBranchId || null;
    if (!branchId && trainer_id) {
      const trainerBranch = await db.query(
        `SELECT branch_id FROM trainers WHERE id = $1`,
        [trainer_id]
      );
      branchId = trainerBranch.rows[0]?.branch_id || null;
    }
    if (!branchId) {
      const error = new Error('branch_id is required (or assign a trainer with a branch)');
      error.status = 400;
      error.errorCode = 'MISSING_BRANCH_ID';
      return next(error);
    }

    // Capacity = sum of branch vehicle max_per_slot (or provided overrides) — not a hardcoded default
    ...
    const finalCapacity = Math.max(1, totalCapacity);
    ...
    // Slot visibility: within configured booking window (settings.booking_window_hours)
    let isVisible = true;
    if (start_time) {
      const slotStartTime = new Date(start_time);
      const visibilityHours = await bookingWindowSvc.getBookingWindowHours();
      const visibilityThreshold = new Date(Date.now() + visibilityHours * 60 * 60 * 1000);
      const now = new Date();
    ...
    const result = await db.query(`
      INSERT INTO slots (trainer_id, start_time, end_time, capacity, status, slot_date, is_auto_generated, is_visible, branch_id)
...
router.put('/:id', ...adminAccess('slots', 'edit'), validateSlotUpdate, async (req, res, next) => {
  try {
    ...
    // Update visibility if start_time is being changed (configured booking window)
    if (start_time !== undefined) {
      const slotStartTime = new Date(start_time);
      const visibilityHours = await bookingWindowSvc.getBookingWindowHours();
      const visibilityThreshold = new Date(Date.now() + visibilityHours * 60 * 60 * 1000);
...
router.post('/generate', ...adminAccess('slots', 'create'), async (req, res, next) => {
  try {
    const { date, force, branch_id } = req.body;
...
router.post('/update-visibility', ...adminAccess('slots', 'edit'), async (req, res, next) => {
  try {
    ...
    if (hasFunction) {
      ...
      // Booking window: visible when slot is in the future and within configured hours
      const visibilityHours = await bookingWindowSvc.getBookingWindowHours();
      const result = await db.query(`
        UPDATE slots
        SET is_visible = (
          start_time > NOW()
          AND start_time <= (NOW() + INTERVAL '${visibilityHours} hours')
        )
