...
router.post('/login', loginLog, loginLimit, validateLogin, async (req, res, next) => {
  ...
  try {
    ...
    await auditService.logAuthEvent(user.id, 'LOGIN_SUCCESS', { email, role: user.role }, ip, req.headers['user-agent']);
...
router.post('/logout', authenticate, async (req, res) => {
  const ip = getClientIp(req);
  await auditService.logAuthEvent(req.user.id, 'LOGOUT', { email: req.user.email }, ip, req.headers['user-agent']);
  clearAuthCookie(res);
  req.logout(() => {
